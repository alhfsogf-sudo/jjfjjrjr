...
class KeyRotator:
    def __init__(self, api_keys: list):
        if not api_keys:
            raise RuntimeError("لا يوجد أي مفتاح Cerebras API معرّف بمتغيرات البيئة.")
        self.current_index = 0
        self.clients = [OpenAI(api_key=k, base_url=config.CEREBRAS_BASE_URL) for k in api_keys]
...
class Architect(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.ai = KeyRotator(config.CEREBRAS_API_KEYS)
